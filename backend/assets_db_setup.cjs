...
async function setupAssetsDB() {
    const connection = await mysql.createConnection({
        host: 'localhost',
        user: 'root',
        password: '',
        database: 'techmicra_erp'
    });

    console.log("Connected to DB");

    await connection.execute(`DROP TABLE IF EXISTS asset_additions`);
    await connection.execute(`DROP TABLE IF EXISTS asset_allocations`);
    await connection.execute(`DROP TABLE IF EXISTS asset_sales`);
    await connection.execute(`DROP TABLE IF EXISTS asset_depreciations`);
    await connection.execute(`DROP TABLE IF EXISTS asset_masters`);
    ...
    `);
    console.log("Created asset_masters");
    ...
    `);
    console.log("Created asset_additions");
    ...
    `);
    console.log("Created asset_sales");
    ...
    `);
    console.log("Created asset_depreciations");

    await connection.end();
}

setupAssetsDB().catch(console.error);
